<template>
  <div class="flex justify-between items-center m-2 px-2">
    <div class="flex items-center px-2">
      ...
      <div class="px-2 text-base text-left font-light">
        <slot></slot>
      </div>
    </div>
    <base-card-icon v-if="hasVoted"></base-card-icon>
  </div>
</template>

<script>
import BaseCardIcon from "../ui/BaseCardIcon.vue";
import { computed } from "vue";
export default {
  components: { BaseCardIcon },
  props: {
    role: String,
    voted: Boolean,
  },
  setup(props) {
    const isModerator = computed(() => props.role === "Moderator");
    const hasVoted = computed(() => props.voted);
    return {
      isModerator,
      hasVoted,
    };
